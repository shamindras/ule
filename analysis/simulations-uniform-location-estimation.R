...
conv_nonagon_p10_plt_rev <- ggplot2::ggplot() +
    ...
    ggplot2::geom_sf(data = sf::st_centroid(conv_nonagon_p10$K_star),
                     color = "darkgreen",
                     fill = "darkgreen", size = 1.5) +
    ggplot2::geom_sf(data = conv_nonagon_p10$K_star_t_s, fill = NA,
                     linetype = "dashed", color = "#D0021C", size = 1) +
...
comb_conv_poly_n_plt <- patchwork::wrap_plots(x = conv_poly_n_plt, nrow = 2)
comb_conv_poly_n_plt
cowplot::ggsave2(plot = comb_conv_poly_n_plt,
...
# Produce a more lightweight TikZ representation of plot
tikz(file = here::here(plot_dir, "comb_conv_poly_n_plt.tex"),
     width = 4,
     height = 7)
print(comb_conv_poly_n_plt)
dev.off()
# remove all lines that invisibly mess up the bounding box
lines <- readLines(con=here::here(plot_dir, "comb_conv_poly_n_plt.tex"))
lines <- lines[-which(grepl("\\path\\[clip\\]*", lines,perl=F))]
lines <- lines[-which(grepl("\\path\\[use as bounding box*", lines,perl=F))]
writeLines(lines,con=here::here(plot_dir, "comb_conv_poly_n_plt.tex"))
